...
if %isOpenBcompare% equ true (
	...
	for /f "delims=" %%i in ('type "C:\Project\JavaProject\config.ini"^| find /i "="') do set %%i
	start "" /min "!bcompareExe!" "MOADomsEE <--> workspace_MOA_COA"
	"!bcompareExe!" "MOADomsEE <--> workspace_MOA_TIPO"
	"!bcompareExe!" "MOADomsEE <--> workspace_MOA_CI"
	"!bcompareExe!" "MOADomsEE <--> workspace_MOA_農保基金"
	"!bcompareExe!" "MOADomsEE <--> workspace_MOA_MOEA"
	"!bcompareExe!" "OASystemEE <--> workspace_OASystem_JDK17"
